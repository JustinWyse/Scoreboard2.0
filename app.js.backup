...
function getCurrentDateRange() {
  ...
  return {
    start: formatDate(priorMonth),
    end: formatDate(priorMonthEnd)
  };
}
...
function getPreviousMonthRange(currentStartDate, currentEndDate) {
  ...
  // Calculate the exact same date range but one month earlier
  ...
  // Handle edge cases for dates that don't exist (e.g., March 31 -> Feb 31 becomes Feb 28/29)
  if (prevStart.getMonth() !== startDate.getMonth() - 1) {
    prevStart.setDate(0); // Go to last day of previous month
  }
  if (prevEnd.getMonth() !== endDate.getMonth() - 1) {
    prevEnd.setDate(0); // Go to last day of previous month
  }
...
function updateKPIs() {
  ...
  const previousMonthData = globalData.filter(row => {
    ...
    if (selectedFacilities.length > 0 && !selectedFacilities.includes(row['facility_id'])) return false;
  ...
  const getChangeText = (change, hasData = true) => {
    if (!hasData) {
      return `<span class="trend-percentage trend-neutral">No prev data</span>`;
    }
    const absChange = Math.abs(change);
    if (change > 0) {
      return `<span class="trend-percentage trend-up">+${absChange.toFixed(1)}% vs prev month</span>`;
    } else if (change < 0) {
      return `<span class="trend-percentage trend-down">-${absChange.toFixed(1)}% vs prev month</span>`;
    } else {
      return `<span class="trend-percentage trend-neutral">0.0% vs prev month</span>`;
    }
  };
  ...
  if (kpiContainer) {
    kpiContainer.innerHTML = `
      <div class="kpi ${getTrendClass(total_attendees, 1000)}">
        ...
        <div class="kpi_change">${getChangeArrow(attendees_change)} ${getChangeText(attendees_change, hasPreviousData)}</div>
      ...
      <div class="kpi ${getTrendClass(total_bookings, 1200)}">
        ...
        <div class="kpi_change">${getChangeArrow(bookings_change)} ${getChangeText(bookings_change, hasPreviousData)}</div>
      ...
      <div class="kpi ${getTrendClass(avg_attendance, target_avg_attendance)}">
        ...
        <div class="kpi_change">${getChangeArrow(avg_attendance_change)} ${getChangeText(avg_attendance_change, hasPreviousData)}</div>
      ...
      <div class="kpi trend-neutral">
        ...
        <div class="kpi_change">${getChangeArrow(unique_classes_change)} ${getChangeText(unique_classes_change, hasPreviousData)}</div>
      ...
      <div class="kpi trend-neutral">
        ...
        <div class="kpi_change">${getChangeArrow(unique_participants_change)} ${getChangeText(unique_participants_change, hasPreviousData)}</div>
...
function getTopPerformingClasses(limit = 10) {
  ...
  filteredData.forEach(r => {
    const className = r['class_name'];
    const facilityId = r['facility_id'];
...
function createMultiFacilityTimeSeriesChart(selectedFacilities, groupBy) {
  ...
  selectedFacilities.forEach((facilityId, index) => {
    const facilityData = filteredData.filter(r => r.facility_id === facilityId);
...
function applyFilters() {
  ...
  filteredData = globalData.filter(row => {
    ...
    if (selectedFacilities.length > 0 && !selectedFacilities.includes(row['facility_id'])) keep = false;
...
function initializeDashboard() {
  ...
  const facilities = uniqueSorted(globalData.map(r => r['facility_id']).filter(Boolean));
...
function generateInsights() {
  ...
  const insights = [
    ...
    `🏢 ${new Set(filteredData.map(r => r.facility_id)).size} facilities are active`,
