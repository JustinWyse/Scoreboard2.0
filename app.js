    ...
    categories.forEach((category, index) => {
      ...
      if (includeYoY) {
        ...
        if (categoryComparisonData.length > 0) {
          ...
          const adjustedDates = comparisonAggregated.dates.map((date, idx) => aggregated.dates[idx] || date);

          traces.push({
            x: adjustedDates,
            y: comparisonAggregated.date_vals,
            type: 'scatter',
            mode: 'lines+markers',
...
function updateCharts() {
  ...
  if ((compareYearOverYear || compareTwoYearsAgo) && getCurrentDateRange().hasSelection) {
    ...
    createSideBySideBarCharts('facility_pie_chart',
      { labels: aggregated.fac_names.slice(0, 10), values: aggregated.fac_vals.slice(0, 10), title: 'Current Period' },
