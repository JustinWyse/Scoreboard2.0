...
function createTimeSeriesWithYoY(elementId, currentData, comparisonData, groupBy, comparisonLabel) {
  ...
  try {
    ...
    if (comparisonData && comparisonData.dates && comparisonData.dates.length > 0) {
      ...
      const adjustedDates = comparisonData.dates.map((date, idx) => {
        if (currentData.dates[idx]) {
          return currentData.dates[idx];
        }
        return date;
      });
...
function createMultiCategoryTimeSeries(elementId, categories, groupBy) {
  const element = safeGetElement(elementId);
  ...
  try {
    ...
    const layout = {
      title: {
        text: `Category Comparison: ${categories.join(' vs ')}`,
        font: { color: '#f0f6fc', size: 18 }
      },
      paper_bgcolor: '#161b22',
      plot_bgcolor: '#161b22',
...
function updateCharts() {
  ...
  if (selectedCategories.length > 1) {
    ...
    createMultiCategoryTimeSeries('ts_chart', selectedCategories, groupBy);
  ...
  const topInstructorTotals = aggregated.instr_totals?.slice(0, 10).reverse() || [];
  ...
  // NEW: Instructor pie chart with YoY
  if ((compareYearOverYear || compareTwoYearsAgo) && getCurrentDateRange().hasSelection) {
    const yearsBack = compareTwoYearsAgo ? 2 : 1;
    const comparisonData = getComparisonData(yearsBack);
    const comparisonAggregated = aggregate(comparisonData, groupBy);
    const comparisonLabel = compareTwoYearsAgo ? '2 Years Ago' : '1 Year Ago';
    createPieChart('instructor_pie_chart', 
      { labels: topInstructors, values: topInstructorTotals },
      'Top Instructors Distribution',
      { labels: comparisonAggregated.instr_names?.slice(0, 10), values: comparisonAggregated.instr_totals?.slice(0, 10) },
      comparisonLabel
    );
  } else {
    createPieChart('instructor_pie_chart', { labels: topInstructors, values: topInstructorTotals }, 'Top Instructors Distribution');
  }
  ...
  if (topClasses.length > 0) {
    ...
    // NEW: Class pie chart with YoY
    if ((compareYearOverYear || compareTwoYearsAgo) && getCurrentDateRange().hasSelection) {
      const yearsBack = compareTwoYearsAgo ? 2 : 1;
      const comparisonData = getComparisonData(yearsBack);
      const comparisonTopClasses = getTopPerformingClassesFromData(comparisonData, 10);
      const comparisonLabel = compareTwoYearsAgo ? '2 Years Ago' : '1 Year Ago';
      createPieChart('class_pie_chart',
        { labels: topClasses.map(cls => cls.className).slice(0, 10), values: topClasses.map(cls => cls.totalAttendance).slice(0, 10) },
        'Top Classes Distribution',
        { labels: comparisonTopClasses.map(cls => cls.className), values: comparisonTopClasses.map(cls => cls.totalAttendance) },
        comparisonLabel
      );
    } else {
      createPieChart('class_pie_chart', { labels: topClasses.map(cls => cls.className).slice(0, 10), values: topClasses.map(cls => cls.totalAttendance).slice(0, 10) }, 'Top Classes Distribution');
    }
  }
  ...
  // NEW: Facility pie chart with YoY
  if ((compareYearOverYear || compareTwoYearsAgo) && getCurrentDateRange().hasSelection) {
    const yearsBack = compareTwoYearsAgo ? 2 : 1;
    const comparisonData = getComparisonData(yearsBack);
    const comparisonAggregated = aggregate(comparisonData, groupBy);
    const comparisonLabel = compareTwoYearsAgo ? '2 Years Ago' : '1 Year Ago';
    createPieChart('facility_pie_chart',
      { labels: aggregated.fac_names.slice(0, 10), values: aggregated.fac_vals.slice(0, 10) },
      'Facility Distribution',
      { labels: comparisonAggregated.fac_names.slice(0, 10), values: comparisonAggregated.fac_vals.slice(0, 10) },
      comparisonLabel
    );
  } else {
    createPieChart('facility_pie_chart', { labels: aggregated.fac_names.slice(0, 10), values: aggregated.fac_vals.slice(0, 10) }, 'Facility Distribution');
  }
...
function createCategoryPieChart() {
  ...
  filteredData.forEach(r => {
    const category = r['greatgrandparent_category'] || 'Unknown';
  ...
  sections.forEach(section => {
    ...
    header.addEventListener('click', () => {
      ...
      if (content) {
        ...
        collapseBtn.title = isCollapsed ? 'Collapse section' : 'Expand section';
      }
    });
  });
}
