...
// Check admin access and hide/show export buttons
function checkAdminAccess() {
  const isAdmin = window.isAdmin || false;
  const exportCsvBtn = safeGetElement('export_csv');
  const exportJsonBtn = safeGetElement('export_json');
  
  if (!isAdmin) {
    // Hide export buttons for non-admin users
    if (exportCsvBtn) exportCsvBtn.style.display = 'none';
    if (exportJsonBtn) exportJsonBtn.style.display = 'none';
    console.log('📊 Standard access - Export features disabled');
  } else {
    console.log('🔑 Admin access - All features enabled');
  }
}
